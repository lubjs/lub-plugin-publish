...
class PublishCommand extends Command {
  ...
  /* istanbul ignore next */
  async preReleaseCheck(context, npm) {
    const ypkgfilesArgs = [
      '--entry',
      'app',
      '--entry',
  ...
  async run(context) {
    let { registry, filename, client, npm } = context.argv;
    this.prepare(context);

    // check for private: true
    if (npm) {
      npm = !this.pkg.private;
    }
    ...
    const answers = await this.preReleaseCheck(context, npm);
